...
# Fold-level inferential tests are intentionally disabled. The outer validation
# protocols contain only a small number of structured partitions, so Kruskal-Wallis
# and Wilcoxon tests on fold scores are not used for manuscript claims. Use
# make_performance_statistics.py for paired prediction-level, cluster-aware tests.
STATISTICAL_GROUPS = {}
...
def save_paper_tables_excel(output_tables, stats_tables, group_name, out_path):
    ...
    with pd.ExcelWriter(out_path) as writer:
        ...
        if kruskal_df.empty and pairwise_df.empty:
            pd.DataFrame([{
                "note": (
                    "Fold-level inferential tests are disabled because the validation "
                    "protocols contain only a small number of structured partitions. "
                    "Use Supplementary File S3 generated by make_performance_statistics.py."
                )
            }]).to_excel(writer, sheet_name="statistical_tests_note", index=False)
        if not pairwise_df.empty:
            pairwise_df[[
                "experiment", "experiment_title", "comparison_group", "metric",
...
def main():
    ...
    print(
        "Fold-level inferential tests are disabled. "
        "Run make_performance_statistics.py for paired cluster-aware comparisons."
    )
