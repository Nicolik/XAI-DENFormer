...
def build_region_gxi_long_df(input_dir, regions, class_dict, region_reduce='mean', normalize_per_sample=True):
    import re
    import numpy as np
    ...
    rx = re.compile(r'test_batch(\d+)_sample(\d+)_class(\d+)_target(\d+)_pred(\d+)_gxi\.npy$')
    rows = []
    for fname in sorted(os.listdir(input_dir)) if os.path.isdir(input_dir) else []:
        m = rx.match(fname)
        if not m:
            continue
        batch, sample, class_idx, target, pred = map(int, m.groups())
        arr = np.load(os.path.join(input_dir, fname)).astype(float, copy=False)
        ...
        for region_name, (start, end) in regions.items():
            ...
            rows.append({
                ...
                'serotype': serotype,
                'region': region_name,
                'gxi': float(reduce_fn(arr[start:end])),
                'file': fname,
                'target': target,
                'pred': pred,
            })
...
def _run_box_analysis(df_long, out_dir, regions_order, label):
    ...
    csv_path = os.path.join(out_dir, 'gxi_by_region_long.csv')
...
def run_fold(args, fold, model_dir, xai_out_root):
    ...
    if npz_path is not None:
        ...
        df_long = build_region_gxi_long_df(
            input_dir=npy_dir,
            regions=regions,
            class_dict=CLASS_DICT,
            region_reduce='mean',
...
def run_dataset(args, folds, model_dir, xai_out_root):
    ...
    for fold in folds:
        fold_id_safe = safe_name(fold['fold_id'])
        fold_dir_name = f'split_{fold_id_safe}'
        split_dir = os.path.join(model_dir, 'gxi', fold_dir_name)
        npy_dir = os.path.join(split_dir, 'numpy')
        npz_path = find_split_npz(split_dir, 'gxi', subset='test')

        if npz_path is None and not os.path.isdir(npy_dir):
            print(f'[WARN] Missing GxI npz/numpy dir, skipping fold {fold_id_safe}: {split_dir}')
        ...
        if npz_path is not None:
            ...
            df_fold = build_region_gxi_long_df(
                input_dir=npy_dir,
                regions=regions,
                class_dict=CLASS_DICT,
                region_reduce='mean',
    ...
    if not dfs:
        print('[WARN] No fold-level GxI files found. Dataset-level output-box was not generated.')
        return None
...
def main():
    ...
    xai_out_root = build_xai_output_dir(paths.logs_dir, 'gxi_box', 'denformer', args.pooling, k_type, run_suffix, args.epochs)
    ...
    summary_path = os.path.join(xai_out_root, 'gxi_box_summary.csv')
